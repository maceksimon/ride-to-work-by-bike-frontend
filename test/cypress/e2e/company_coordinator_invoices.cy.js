...
describe('Company coordinator invoices page', () => {
  ...
  context('PDF generation - polling on mounted', () => {
    beforeEach(() => {
      cy.viewport(1920, 2500);
      // set system time to be in the correct active token window
      cy.clock(systemTimeChallengeActive, ['Date']).then(() => {
        cy.task('getAppConfig', process).then((config) => {
          ...
          cy.window().then((win) => {
            ...
            cy.interceptCoordinatorInvoicesGetApi(
              config,
              'apiGetCoordinatorInvoicesResponseMissingTwoPdfs.json',
            );
            cy.visit(
              '#' + routesConf['coordinator_invoices']['children']['fullPath'],
            );
    ...
    it('shows loading state for missing PDFs and refetches invoices periodically', () => {
      cy.get('@config').then((config) => {
        // check that initial admin organisation response is loaded
        cy.waitForCoordinatorInvoicesGetApi(
          'apiGetCoordinatorInvoicesResponseMissingTwoPdfs.json',
        ...
        cy.get('@getCoordinatorInvoices.all').should('have.length', 1);
        // test initial table data
        cy.fixture('apiGetCoordinatorInvoicesResponseMissingTwoPdfs.json').then(
          (response) => {
            const { invoices } = response.results[0];
            const filteredInvoices = invoices.filter(
              (invoice) => invoice.fakturoid_invoice_url === '',
            );
            cy.dataCy('table-invoices-row').should(
              'have.length',
              invoices.length,
            ...
            cy.dataCy('table-invoices-generating-spinner')
              .should('exist')
              .and('be.visible')
              .and('have.length', filteredInvoices.length);
          },
        );
        // override intercept
        cy.interceptCoordinatorInvoicesGetApi(
        ...
        cy.get('@getCoordinatorInvoices.all').should('have.length', 2);
        cy.fixture('apiGetCoordinatorInvoicesResponseMissingOnePdf.json').then(
          (response) => {
            const { invoices } = response.results[0];
            const filteredInvoices = invoices.filter(
              (invoice) => invoice.fakturoid_invoice_url === '',
        ...
        cy.get('@getCoordinatorInvoices.all').should('have.length', 3);
        cy.fixture('apiGetCoordinatorInvoicesResponse.json').then(
          (response) => {
            const { invoices } = response.results[0];
            cy.dataCy('table-invoices-row').should(
              'have.length',
